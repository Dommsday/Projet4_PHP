<form action="" method="post">
  <p>
    ...
    <label>Auteur</label>
    <input type="text" name="auteur" value="<?= isset($post) ? $post['auteur'] : '' ?>" /><br />
    
    <?= isset($erreurs) && in_array(\Entity\News::TITRE_INVALIDE, $erreurs) ? 'Le titre est invalide.<br />' : '' ?>
    <label>Titre</label><input type="text" name="titre" value="<?= isset($post) ? $post['titre'] : '' ?>" /><br />
    
    <?= isset($erreurs) && in_array(\Entity\News::CONTENU_INVALIDE, $erreurs) ? 'Le contenu est invalide.<br />' : '' ?>
    <label>Contenu</label><textarea rows="8" cols="60" name="contenu"><?= isset($post) ? $post['contenu'] : '' ?></textarea><br />
<?php
